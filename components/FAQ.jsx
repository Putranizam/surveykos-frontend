  ...
  return (
    <section id="faq" className="mx-auto w-full max-w-6xl px-4 py-18 md:px-6">
      <div className="mb-10 text-center">
        <h2 className="text-2xl font-bold tracking-tight md:text-4xl">Pertanyaan yang Sering Diajukan</h2>
      ...
      <div className="space-y-3">
        {FAQ_ITEMS.map((item, index) => {
          ...
          return (
            <article key={item.question} className="rounded-2xl border border-zinc-200 bg-white shadow-sm">
              <button
                type="button"
                onClick={() => setOpenIndex(isOpen ? -1 : index)}
                className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left"
              >
                <span className="text-sm font-semibold text-zinc-900 md:text-base">{item.question}</span>
                <ChevronDown
              ...
              >
                <div className="min-h-0">
                  <p className="px-5 pb-5 text-sm leading-6 text-zinc-600 md:text-[15px]">{item.answer}</p>
                </div>
              </div>
            </article>
          );
